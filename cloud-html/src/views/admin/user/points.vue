...
<script>
import { addPointsAdminApi, payPointsAdminApi } from '@/api/pay.js'
...
export default {
  ...
  methods: {
    async fetchData () {
      this.loading = true
      try {
        const resp = await payPointsAdminApi(this.fetchDataParams)
    ...
    async submitAdjust () {
      ...
      const resp = await addPointsAdminApi(this.adjustForm)
      await this.fetchData()
