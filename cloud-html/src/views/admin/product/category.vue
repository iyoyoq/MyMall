...
import { productCategoryCreateAdminApi, productCategoryListApi, productCategoryUpdateAdminApi } from '@/api/product.js'
  ...
  methods: {
    ...
    async handleSubmit (done) {
      this.$refs.formRef?.validate(async (errors) => {
        ...
        try {
          if (isEdit) {
            await productCategoryUpdateAdminApi(this.form)
          } else {
            await productCategoryCreateAdminApi(this.form)
          }
    ...
    async handleDelete (record) {
      try {
        await productCategoryUpdateAdminApi({
          id: record.id,
          status: -1
        })
        this.keySelector++
